...
import (
	...
	cni_network "github.com/emunet/emunet-operator/emu-cni/cni-plugin/cni-network"
	config "github.com/emunet/emunet-operator/emu-cni/tools/config"
	ebpftc "github.com/emunet/emunet-operator/emu-cni/tools/ebpf/ebpf-tc"
)
...
func (e *EmuCNIPlugin) Add(args *skel.CmdArgs) error {
	...
	// [进入容器空间]
	if setErr := targetNs.Set(); setErr != nil {
		return fmt.Errorf("failed to enter netns: %v", setErr)
	}
